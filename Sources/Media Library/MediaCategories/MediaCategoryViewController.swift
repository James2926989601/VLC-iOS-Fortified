    ...
    override func viewDidLoad() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if #available(iOS 26.0, visionOS 26.0, *) {
                // iOS 26 uses the UISearchController integrated in the navigation
                // item. Never mount the legacy standalone UISearchBar as well.
            } else {
                self.setupSearchBar()
                if isAlbum {
    ...
    override func viewDidDisappear(_ animated: Bool) {
        ...
        if let model = model as? CollectionModel,
           model.mediaCollection is VLCMLAlbum {
            ...
            // Album pages never display the legacy standalone search bar. Readding
            // it here left it unconstrained over the back button after returning.
