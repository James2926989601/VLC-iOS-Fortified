...
#define VLC_ORIGINAL_PIXEL_SIZE 0.f
...
{
    return [VLCThumbnailsCache thumbnailForURL:url maxPixelSize:VLC_ORIGINAL_PIXEL_SIZE];
}
...
{
    if (maxPixelSize <= VLC_ORIGINAL_PIXEL_SIZE) {
        return [NSString stringWithFormat:@"%@|original", url.path];
    }
...
- (UIImage *)_thumbnailForURL:(NSURL *)url maxPixelSize:(CGFloat)maxPixelSize
{
    ...
    theImage = [self imageAtPath:path maxPixelSize:maxPixelSize];
    if (!theImage) {
        return nil;
    }
...
- (UIImage *)imageAtPath:(NSString *)path maxPixelSize:(CGFloat)maxPixelSize
{
    if (maxPixelSize <= VLC_ORIGINAL_PIXEL_SIZE) {
